      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          if (x + u >= 0 && x + u < width && y + v >= 0 && y + v < height) {
            HLS_UNROLL_LOOP(CONSERVATIVE,MASK_X,"THIS");
            R = *(source_bitmap +
...
void Testbench::fetch_result() {
 ...
 i_resultb.reset();
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
#ifndef NATIVE_SYSTEMC
			resultr = i_resultr.get();
      resultg= i_resultg.get();
      resultb= i_resultb.get();
#else
			resultr = i_resultr.read();
      resultg = i_resultg.read();
      resultb = i_resultb.read();
#endif


        // black
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = resultr;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = resultg;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = resultb;
